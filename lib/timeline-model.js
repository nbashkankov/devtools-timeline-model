...
requireval("./lib/number-stubs.js");
...
class SandboxedTracingModel {
  ...
  getRangeStats(startTime, endTime) {
    var model = this._timelineModel;
    var aggregatedStats = {};
    ...
    aggregatedStats['range'] = Common.UIString('Range:  %s \u2013 %s', Number.millisToString(startOffset), Number.millisToString(endOffset));
    return aggregatedStats;
  }
}

var sandboxedTracingModel = new SandboxedTracingModel();

class SandoxedCpuProfileModel {
  init(events) {
    this.model = new SDK.CPUProfileDataModel(events);
  }
...
var sandboxedCpuProfileModel = new SandoxedCpuProfileModel();
